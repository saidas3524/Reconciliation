...
import { GET_ALL_USERS, setAllUsers } from "../actions/setUserInfo";

const data = {
  UserID: 2381996,
  LastName: "Nettem",
  FirstName: "Goutam Kumar",
  UserName: "oemdevsnew@live.com",
  CompanyName: "MICROSOFT (THAILAND) LTD KOMIN LINPHRACHAYA",
  Email: "v-gonett@microsoft.com",
  UserCustomerGroups: [
    "BNG",
    "CMT",
    "MDB",
  ...
  HomeCompanyAccountGroupCode: "Z4IC",
  Name: "Goutam Kumar Nettem"
};
export function* userInfoSaga() {
    try {

        const {publicKey} = yield take(GET_USER_INFO);

        let responseC = yield call(InvokeUrl, `${API_ENDPOINT}/users/logged/${publicKey}`, 'GET');
...
export function* usersInfoSaga() {
    try {

         yield take(GET_ALL_USERS);
        // yield put(setUser({user:{FirstName:"Sai Krishna",LastName:"Dasoju",Email:"dasojusaikrishna3524@mail.com"}}))
        // yield put(setUser(data));
        // yield put(alertAddAction({type:"information",message:"fetched user details"}));

        let responseC = yield call(InvokeUrl, `${API_ENDPOINT}/users/all`, 'GET');

        if (responseC.status == 200) {
            const data = yield apply(responseC, responseC.json);
            let users = {};
            data.map((d)=>{
                users[d.publicKey] = d.name;
            });
            ...
            yield put(alertAddAction({ type: AlertsTypes.ERROR, message: ["Failed to fetch users details."] }));
        }
